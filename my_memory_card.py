...
def checkAnswer():
    if list_btns[0].isChecked():
        ...
        answer.setText('Количество правильных ответов '+ str(window.correct))
    else:
        answer.setText('Неправильный ответ')
        window.incorrect = window.incorrect + 1
        answer.setText('Количество неправильных ответов '+ str(window.incorrect))
    group.hide()
    groupok.show()
...
qlist = [
    Question('Как дела? Загадка Жака Фреско', 'машина', 'эта новый кадилак', 'человек', 'нет'),
    Question('в чем сила', 'в теле', 'в духе', 'в братьях', 'в друзьях'),
    Question('сколько мне лет', 'мне 5', 'я молодой', 'я старый', 'нет'),
    Question('зачем?', 'зотем', 'затем', 'зотем', 'затем'),
    Question('ты чо?', 'ujgybr', 'ujgybr', 'гопник', 'гопник'),
    Question('ответьте правильно, если не ответите то слиты', 'отыечу конечно', 'слишком сложно', 'я бот', 'нет'),
    Question('вкусно?', 'неит', 'я лублю какахи', 'пилимешки', 'макароны')
]
ask(qlist[0])
